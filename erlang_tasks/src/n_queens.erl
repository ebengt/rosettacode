...
solve( N ) ->
    ...
    Diagonals = [{X, diagonals(N, X)} || X <- Positions],
    try
    bt( {N, Diagonals},  Positions, [] )
...
is_all_queens( {N, _Diagonals}, Board ) -> N =:= erlang:length( Board ).

is_diagonal( _N, [] ) -> false;
is_diagonal( {_N, Diagonals}=N, [Position | T] ) ->
	T =/= (T -- proplists:get_value( Position, Diagonals ))
